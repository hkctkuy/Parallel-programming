...
8) Многопоточная реализация итерационного метода решения гиперболического
уравнения в частных производных на трехмерном прямоугольном параллелепипеде
(вариант 3) с общей памятью (OpenMP) и разделяемой памятью (MPI)
...
Опции:
    ...
    $ Usage: ./solver N px py pz
    $ N is a Node Number per Side
    $ px is a x axis decomposition param
    $ py is a y axis decomposition param
    $ pz is a z axis decomposition param

Изменение количество OpenMP нитей нужно делать с помощью переменной
`OMP_NUM_THREADS`.

Для измерения параллельного ускорения на Полюсе можно использовать готовый скрипт с
указанными размерностями, модифицируя его под свои нужды:

	$ ./run.sh

9) Многопоточная реализация итерационного метода решения гиперболического
уравнения в частных производных на трехмерном прямоугольном параллелепипеде
(вариант 3) с общей памятью (OpenMP) и разделяемой памятью (MPI), а также с
поддержкой вычислений на графическом ускорителе (CUDA).
...
Опции:
    ...
    $ Usage: ./solver N px py pz [cpu|gpu]
    $ N is a Node Number per Side
    $ px is a x axis decomposition param
    $ py is a y axis decomposition param
    $ pz is a z axis decomposition param
