...
\Year{2024}
  ...
  \TitleExtraBottom{для кластерных систем с распределенной памятью}
...
\begin{enumerate}
  ...
  \item \textbf{Comm}~--- построение схемы обменов;
...
\begin{itemize}
  ...
  \item $K_1, K_2$~--- параметры для количества треугольных и четырехугольных
    элементов;
  \item $P_x, P_y$~--- параметры декомпозиция по двум осям (Программа
    запускается через MPI с числом процессов $P = P_x * P_y$).
\end{itemize}

\textbf{Выходные данные:}
\begin{itemize}
  \item $N$~--- размер матрицы (число вершин в графе);
  \item $N_o$~--- число собственных вершин в локальной подобласти;
  \item $IA, JA$~--- портрет разреженной матрицы смежности графа, (в формате
    CSR);
  \item $Part$~--- массив с номерами владельцев по каждой вершине в локальном графе;
  \item $L2G$~--- массив с номерами вершин локального графа в глобальном графе,
    то есть отображение из локальной нумерации в глобальную.
\end{itemize}
...
\subsubsection{Построение схемы обмена данными}
...
\textbf{Выходные данные:}
\begin{itemize}
  \item $Comm$~--- схема обменов, в данном случае массив специальных структур
    (подробнее в приложении~\ref{sec:appendix} в листинге~\ref{list:comm_str}).
\end{itemize}
...
\begin{itemize}
  ...
  \item build\_comm~--- функция построения схемы обменов;
...
Используемые опции компиляции: \textit{-std=c++11 -O3 -fopenmp}
...
По таб. \ref{tab:process_paral} и рис. \ref{fig:process_step} и
\ref{fig:process_func} хорошо видно наличие параллельного ускорения для разных
этапов программы и основных функций соответственно при увеличении числа
процессов (каждый процесс использовал при этом одну нить). На таб.
\ref{tab:thread_paral} и рис.  \ref{fig:thread_step} и \ref{fig:thread_func}
приведены аналогичные результаты про росте числа нитей для 2 процессов (указано
число нитей каждого процеса). Замеры производились с параметрами $N_x = 8000$ и
$N_y = 8000$ на узлаx \textit{polus-c3-ib} и \textit{polus-c4-ib} и
ядрам.

Исходя из значений в таблицах можно сделать вывод, что данное решение гораздо
лучше распараллеливается при параллелизме с распределённой памятью, чем с общей. 

\begin{table}[h]
  \centering
  \caption{Зависимость времени работы этапов программы и основных функций от
  числа параллельных процессов}
  \label{tab:process_paral}
  \includegraphics[width=\textwidth]{images/process_parallel_acceleration.pdf}
\end{table}

\begin{figure}[h]
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/process_step_time.png}
     \caption{Зависимость времени работы этапов программы от числа параллельных
     процессов}
     \label{fig:process_step}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/process_func_time.png}
     \caption{Зависимость времени работы основных функций от числа параллельных
     процессов}
     \label{fig:process_func}
   \end{minipage}
\end{figure}
...
\begin{table}[h]
  \centering
  \caption{Зависимость времени работы этапов программы и основных функций от
  числа нитей процесса}
  \label{tab:thread_paral}
  \includegraphics[width=\textwidth]{images/thread_parallel_acceleration.pdf}
\end{table}

\begin{figure}[h]
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/thread_step_time.png}
     \caption{Зависимость времени работы этапов программы от числа нитей
     процесса}
     \label{fig:thread_step}
   \end{minipage}\hfill
   \begin{minipage}{0.48\textwidth}
     \centering
     \includegraphics[width=\linewidth]{images/thread_func_time.png}
     \caption{Зависимость времени работы основных функций от числа нитей
     процесса}
     \label{fig:thread_func}
   \end{minipage}
\end{figure}
...
В листинге~\ref{list:comm_str} дано описание структуры для обмена между
процессами.  В листингах~\ref{list:gen}, \ref{list:fill}, \ref{list:comm}
\ref{list:solve}, \ref{list:scal}, \ref{list:add} и \ref{list:mul} приведены
прототипы и описания основных реализованных функций.

\begin{listing}[h]
  \caption{Структура для обмена между процессами}
  \label{list:comm_str}
\begin{minted}{C}
...
\begin{listing}[h]
  \caption{Функция построения схемы обменов}
  \label{list:comm}
\begin{minted}{C}
